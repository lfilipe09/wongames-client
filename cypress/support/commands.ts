...
Cypress.Commands.add('signIn', (email = 'luisinhofinancas@gmail.com', password = 'Strapi123') => {
...
Cypress.Commands.add('shouldRenderBanner', () => {
  //Aqui eu to selecionando o slider
    //o within é pra entrar no contexto do elemento selecionado e agir somente dentro desse escopo
  cy.get('.slick-slider').within(() => {
    cy.findByRole('heading', {name: /inside/i})
    ...
    cy.findByRole('heading', {name: /xcom 2/i})
    cy.findByRole('link', {name: /buy now/i})
...
Cypress.Commands.add('shouldRenderShowcase', ({name, highlight = false}) => {
  cy.getByDataCy(`"${name}"`).within(() => {
    ...
    //procura se os highlights estão ou não estão na pág
    cy.getByDataCy("highlight").should(highlight ? 'exist' : 'not.exist')
    
    //procura se os highlights que existem possuem link
    if(highlight){
